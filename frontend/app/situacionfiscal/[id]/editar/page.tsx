...
import { getSituacionFiscal, updateSituacionFiscal } from '@/app/services/api';
import { ArrowLeft, Save, Loader2 } from 'lucide-react';

const PRIMARY = '#1a6fa0';

export default function EditarSituacionFiscalPage() {
  ...
  useEffect(() => {
    getSituacionFiscal(Number(id))
      .then((res) => setNombre(res.data.nombre ?? ''))
      .catch(() => setLoadError('No se pudo cargar la situación fiscal'))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await updateSituacionFiscal(Number(id), { nombre: nombre.trim() });
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#f4f8fb' }}>
      ...
      <div className="bg-white border-b border-slate-200 sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-xl font-bold text-slate-800">Editar Situación Fiscal</h1>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        {loadError && (
          <div className="bg-red-50 border border-red-200 rounded-xl p-4 text-red-700 text-sm">{loadError}</div>
        )}
