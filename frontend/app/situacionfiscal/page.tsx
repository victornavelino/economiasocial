...
import { getSituacionesFiscales, deleteSituacionFiscal } from '@/app/services/api';
import { Edit, Trash2, Eye, Plus, FileText, Search } from 'lucide-react';

interface SituacionFiscal {
  id: number;
  nombre: string;
}
...
export default function SituacionFiscalPage() {
  const [items, setItems] = useState<SituacionFiscal[]>([]);
  const [filtered, setFiltered] = useState<SituacionFiscal[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await getSituacionesFiscales();
      const data = res.data?.results ?? res.data ?? [];
      setItems(data);
      setFiltered(data);
    } catch {
      setError('Error al cargar las situaciones fiscales');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (confirm('¿Estás seguro de eliminar esta situación fiscal?')) {
      try {
        await deleteSituacionFiscal(id);
        fetchData();
      } catch {
        alert('Error al eliminar. Puede que esté siendo usada por emprendedores.');
      }
    }
  };
    ...
    <div className="min-h-screen" style={{ backgroundColor: '#f4f8fb' }}>
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <Link href="/situacionfiscal/nuevo">
            <button
        ...
        {!loading && !error && filtered.length === 0 && (
          <div className="text-center py-16">
            <FileText className="w-12 h-12 mx-auto mb-3 opacity-30" style={{ color: PRIMARY }} />
            <p className="font-medium" style={{ color: '#5a7a8a' }}>
              {search ? 'Sin resultados para la búsqueda' : 'No hay situaciones fiscales registradas'}
            </p>
          </div>
        )}
              ...
              >
                <div className="p-4 flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <Link href={`/situacionfiscal/${item.id}`}>
                    ...
                    <Link href={`/situacionfiscal/${item.id}/editar`}>
