...
import { z } from 'zod';
import { getServicio, updateServicio, getServicios } from '@/app/services/api';
...
export default function EditarServicio() {
  ...
  useEffect(() => {
    ...
    Promise.all([getServicio(Number(id)), getServicios()])
      .then(([res, listRes]) => {
        const item = res.data;
        reset({
          nombre: item.nombre,
        ...
        setServicios(data.filter((s: any) => s.tipo === 'servicio' && s.id !== Number(id)));
      })
      .catch((err) => {
        console.error(err);
        setSubmitError('No se pudo cargar la información del servicio.');
      })
  ...
  const onSubmit = async (data: FormData) => {
    setSubmitError(null);
    try {
      await updateServicio(Number(id), data);
      ...
      <div className="bg-white border-b border-slate-200 sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-xl font-bold text-slate-800">Editar Servicio</h1>
          </div>
        </div>
      </div>
