...
import { getMedioDePago, updateMedioDePago } from '@/app/services/api';
import { ArrowLeft, Save, Loader2 } from 'lucide-react';

const PRIMARY = '#1a6fa0';

export default function EditarMedioDePagoPage() {
    ...
    useEffect(() => {
        getMedioDePago(Number(id))
            ...
            .catch(() => setLoadError('No se pudo cargar el medio de pago'))
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSubmitting(true);
        try {
            await updateMedioDePago(Number(id), { nombre: nombre.trim() });
            router.push('/mediodepago');
            router.refresh();
        } catch (err: any) {
            ...
            <div className="bg-white border-b border-slate-200 sticky top-0 z-10">
                <div className="max-w-2xl mx-auto px-4 py-4 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <h1 className="text-xl font-bold text-slate-800">Editar Medio de Pago</h1>
                    </div>
                </div>
            </div>
